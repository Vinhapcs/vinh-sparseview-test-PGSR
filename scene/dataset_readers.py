...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    train_split_npy = f"{path}/train_split.npy"
    test_txt = f"{path}/sparse/0/test.txt"
    if not os.path.exists(test_txt):
        test_txt = f"{path}/test.txt"
    train_list = None
    test_list = None
    ...
    elif os.path.exists(train_split_npy):
        split = np.load(train_split_npy)
        if split.dtype == bool:
            train_list = [c.image_name for idx, c in enumerate(cam_infos) if split[idx]]
            test_list = [c.image_name for idx, c in enumerate(cam_infos) if not split[idx]]
        else:
            train_list = [os.path.basename(s).split('.')[0] for s in split]
    elif os.path.exists(test_txt):
        with open(test_txt, "r") as f:
            test_list = [os.path.basename(line.strip()).split(".")[0] for line in f.readlines()]
        train_list = [c.image_name for c in cam_infos if c.image_name not in test_list]
    ...
    if train_list is not None and test_list is not None:
        ...
        train_cam_infos = [c for idx, c in enumerate(cam_infos) if c.image_name in train_list]
        test_cam_infos = [c for idx, c in enumerate(cam_infos) if c.image_name not in train_list]
        print(f"train_cam_infos {len(train_cam_infos)}, test_cam_infos {len(test_cam_infos)}")
    elif eval:
